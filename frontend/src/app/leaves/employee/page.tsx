...
export default function EmployeeLeavesPage() {
  ...
  const fetchAndSelectRequest = async (req: LeaveRequest) => {
    // Just use the request from the list - it already has all the data including populated attachment
  ...
  useEffect(() => {
    ...
    const shouldCalculate =
      editForm.from && editForm.to && user?.id;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Simple single file handling - attachmentId is already set from file upload
  ...
  const startEdit = async (req: LeaveRequest) => {
    ...
    // Get the attachment ID - check if it's an object (populated) or string
    let attachmentId = "";
                      ...
                      {selectedRequest ? (
                        <Card className="bg-white/5 border-white/10 text-white">
                          ...
                          <CardContent className="space-y-2 text-sm text-slate-200">
                            ...
                            <p>
                              Dates: {formatDate(selectedRequest.dates.from)} -{" "}
                              {formatDate(selectedRequest.dates.to)} ({selectedRequest.durationDays} day(s))
                            </p>
                ...
                <form onSubmit={handleEditSubmit} className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 text-sm text-slate-300">
                    {editCalculating ? (
                      ...
                      <>
                        <Badge variant="secondary">Net days: {editNetDays.netDays}</Badge>
                        <span className="text-slate-400">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <Button
                      ...
                      className="bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 flex-1"
                    >
                      Save changes
                    </Button>
                    <Button
